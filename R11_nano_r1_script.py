...
def preprocess_example(
    ...
    # Use the 'problem' key instead of 'question'
    question: str = example["problem"]
    # The answer is the first item in the list
    target_answer: str = example["answer"][0]
    ...
    # Include the target_answer for the reward function
    return {"prompt": prompt, "input_ids": input_ids, "target_answer": target_answer}
...
def math_reward_func(completion: str, target_answer: str) -> float:
    """
    Evaluates the completion by comparing the numerical answer to the target.

    Args:
        completion (str): Generated output from the model.
        target_answer (str): The ground truth numerical answer.

    Returns:
        float: Reward score (1.0 for correct, 0.0 for incorrect).
    """
    try:
        # Synthetically prepend <think> to help regex matching
        completion = "<think>" + completion
        ...
        # Clean the answer by removing commas and convert to float for comparison
        model_answer_clean = re.sub(r"[,']", "", model_answer_str)
        target_answer_clean = re.sub(r"[,']", "", target_answer)

        if abs(float(model_answer_clean) - float(target_answer_clean)) < 1e-4:
            return 1.0
        else:
            return 0.0
    except (ValueError, IndexError):
        # Return 0.0 if parsing or float conversion fails
        return 0.0
...
def compute_reward(completion: str, sample: Dict[str, Any], EOS_TOKEN: str) -> Tuple[float, Dict[str, float]]:
    ...
    correctness_reward = math_reward_func(completion=completion, target_answer=target_answer)
...
def main(rank: int):
    ...
    PROMPT_TEMPLATE = (
        ...
        "Show your work in <think> </think> tags. And return the final numerical answer in "
        "<answer> </answer> tags, for example <answer>1234</answer>."
    )
    ...
    for iteration in trange(begin_iter, NUM_ITERATIONS):
        ...
        if dist.get_rank() == 0:
            ...
            selected_keys = [
                ...
                "train/reward_metrics/correctness_reward",
                "train/response_lengths",
                "eval/rewards",
                "eval/reward_metrics/format_reward",
                "eval/reward_metrics/correctness_reward",
            ]
